...
function parseCCGMarkup(markup) {	
	...
	result["allconstits"] = allconstits;
	//console.log(result);
	
	result["toFlatList"] = function () {	// list of derivation steps, not including the plain sentence itself
		var list = [];
	...
	result["toTree"] = function () {	// WARNING: simply returns the structured derivation 
	...
	result["toTable"] = function () {	// Assuming that the derivation does not contain 
...
$(function () {
	$('pre.language-ccg').each(function (i) {
		try {
